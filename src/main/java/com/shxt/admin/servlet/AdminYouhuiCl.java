...
public class AdminYouhuiCl extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(type.equals("deleteOne")) {
			...
			sqlbook = "update bookshopping set book_is_youhui=0 where bookid='" +bookid+ "'";
			//调用dao层的方法进行删除操作
			...
			if(b == true) {
				...
				out.println("window.open('admin/youhui.jsp','_self');");
				out.println("</script>");
			} else {
				out.println("<script>");
			...
			sql = "update activity set activity_startdate='"+start_time+"',activity_stopdate='"+stop_time+"',youhui_number="+num+",activity_title='"+name+"' where activityid='"+id+"'";
			boolean b = youhui.deleteActivity(sql);
			if(b == true) {
				out.println("<script>");
			...
			request.getRequestDispatcher("admin/addyouhuibook.jsp?num="+n).forward(request, response);
			...
			if(bookid==null) {
				...
				out.println("window.open('admin/addyouhuibook.jsp','_self');");
				out.println("</script>");
			} else {
				for(int i=0;i<bookid.length;i++) {
					System.out.println(bookid[i]);
					//通过id得到图书的名称
					booksname.add(bid.getBookInfo(bookid[i]).getBook_name());
					System.out.println(booksname.get(i));
				}
				request.getSession().setAttribute("booksid", bookid);
				request.setAttribute("booksname", booksname);
				request.getRequestDispatcher("admin/addyouhui.jsp").forward(request, response);
			}
		} else {
			//进行真正的添加优惠图书信息
			...
			for(int i = 0; i < bookid.length; i++) {
				sql = "insert into activity values('"+youhui.getNextActivityId()+"','"+start_time+"','"+stop_time+"',"+num+",5,'youhui','"+bookid[i]+"','"+activityname+"')";
				System.out.println(sql);
				//更新图书信息表
				sqlbook = "update bookshopping set book_is_youhui=1 where bookid in("+bookid[i]+",";
