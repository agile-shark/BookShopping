...
ArrayList<BookOrder> al = order.getAllOrdersByUnPayed(pageNow,pageSize);
UserDAO user = new UserDAO();
//获取订单数量 
int num = order.getNumOrdersByUnPayed();
//计算出总页数
int pageCount = 1;
if(num%6==0) {
  ...
  <tr>
    <td height="321" align="center" valign="top"><table width="98%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        <form action="ModifyOrderAll?type=pay" method="post">
          ...
          <tr>
            <td height="29" colspan="7"> &nbsp;&nbsp;
             <%
            	if(al.size() != 0) {
            		%>
            		<input name="Submit2" type="submit" class="btn_2" value="付 款" style="cursor:pointer;"/>       
            		<%
            	}
            %>
            ...
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;总计<%=num %>个记录 &nbsp;当前第  <%=pageNow %> 页
             &nbsp;&nbsp;&nbsp;&nbsp;
             <a href="SearchOrderType?select=payed&pageNow=1">首 页</a> &nbsp;
             <%
             	if(pageNow>1){
             		%>
             		<a href="SearchOrderType?select=payed&pageNow=<%=pageNow-1 %>">上一页</a> &nbsp;
             		<%
             	}
             %>
              <%
             	if(pageNow<pageCount){
             		%>
             		<a href="SearchOrderType?select=payed&pageNow=<%=pageNow+1 %>">下一页</a> &nbsp; 
             		<%
             	}
             %>
             <a href="SearchOrderType?select=payed&pageNow=<%=pageCount %>">尾 页</a> 
              </td>
            </tr>
        </table>
        </form>
